...
import { Camera, MapPin, CheckCircle2, RefreshCw, Clock, AlertTriangle, ArrowRight, ScanFace, ShieldCheck, X, Satellite, RotateCcw, Map as MapIcon, ExternalLink, Ruler } from 'lucide-react';
...
const TimeClock: React.FC<TimeClockProps> = ({ currentUser, locations, onPunch, onGoToProfile }) => {
  ...
  // Função Haversine blindada contra tipos string/number misturados
  const calculateDistance = (lat1: any, lon1: any, lat2: any, lon2: any) => {
    // Forçar conversão para número para evitar erros de string
  ...
  const startGPS = () => {
    ...
    const id = navigator.geolocation.watchPosition(
      ...
      (err) => {
        ...
        else if (err.code === 2) msg = "Sinal de GPS indisponível. Vá para uma área aberta.";
      ...
      const response = await ai.models.generateContent({
        ...
        config: { 
          ...
          responseSchema: {
            type: Type.OBJECT,
            properties: {
              match: { type: Type.BOOLEAN, description: "True se for a mesma pessoa." },
              clearFace: { type: Type.BOOLEAN, description: "True se o rosto estiver visível." },
              reason: { type: Type.STRING, description: "Motivo se falhar." }
            },
  ...
  const handlePunch = async (type: 'ENTRADA' | 'SAIDA') => {
    ...
    if (!nearestLocation || (minDistance !== null && minDistance > nearestLocation.radius)) {
      ...
      setShowDebug(true); // Força exibição do debug para o usuário entender o erro
      return;
    }
  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-xl border border-amber-100 flex flex-col md:flex-row gap-8">
        ...
        <div className="relative w-64 h-64 bg-slate-900 rounded-[2.5rem] overflow-hidden border-4 border-amber-500 shadow-lg">
          ...
          {status === 'SUCCESS' && (
            <div className="absolute inset-0 bg-emerald-600 flex flex-col items-center justify-center text-white animate-in zoom-in">
              ...
              <p className="mt-2 font-black uppercase text-center px-4 leading-tight">Registrado em<br/>{nearestLocation?.name}</p>
            </div>
          )}
      ...
      <div className="flex-1 space-y-6 flex flex-col justify-center">
        ...
        {/* Painel de Diagnóstico Técnico Aprimorado */}
          ...
          <div className="bg-slate-100 p-4 rounded-2xl text-[10px] font-mono space-y-2 animate-in zoom-in-95 border border-slate-200">
             ...
             {(Math.abs(currentPos.lat - nearestLocation.lat) > 10 || Math.abs(currentPos.lng - nearestLocation.lng) > 10) && (
                <div className="bg-amber-100 text-amber-800 p-2 rounded-lg font-bold">
                   ...
                   ALERTA: A diferença é gigantesca! Verifique se a Latitude/Longitude no cadastro da sede possui o sinal negativo (-).
                </div>
             )}
             ...
             <a 
               ...
               target="_blank" 
               rel="noreferrer"
               className="block text-center bg-blue-600 text-white py-2 rounded-lg font-bold uppercase mt-2 hover:bg-blue-700"
             >
                <span className="flex items-center justify-center gap-2"><ExternalLink size={12}/> Conferir no Google Maps</span>
             </a>
          </div>
        )}
        ...
        <div className="flex gap-3">
          <button 
            ...
            className="flex-1 py-5 bg-emerald-600 text-white rounded-2xl font-black text-lg shadow-lg disabled:opacity-20 active:scale-95 transition-all"
          >
            ENTRADA
          </button>
          <button 
            ...
            className="flex-1 py-5 bg-rose-600 text-white rounded-2xl font-black text-lg shadow-lg disabled:opacity-20 active:scale-95 transition-all"
          >
            SAÍDA
          </button>
        </div>
